...
object LineOfSight {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    ...
    while (i < until) {
      output(i) = math.max(previous, input(i) / i)
      previous = output(i)
      i = i + 1
    }
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float, tree: Tree): Unit = {
    tree match {
      case Leaf(from, until, maxPrevious) =>
        downsweepSequential(input, output, math.max(maxPrevious, startingAngle), from, until)

      case Node(left, right) =>
        parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, math.max(left.maxPrevious, startingAngle), right))
    }
  }

  /** Compute the line-of-sight in parallel. */
  def parLineOfSight(input: Array[Float], output: Array[Float], threshold: Int): Unit = {
    val tree = upsweep(input, from = 0, end = input.length, threshold)
    downsweep(input, output, startingAngle = 0, tree)
  }
}
